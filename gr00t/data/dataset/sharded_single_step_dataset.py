...
def extract_step_data(
    ...
    text_list = language_data[list(language_data.keys())[0]][0]
    assert isinstance(text_list, list), f"Expected text to be a list, got {type(text_list)}"
        ...
        for ep_idx, step_indices in episodes:
            try:
                ...
                for step_index in step_indices:
                    ep_datapoints.append(self.get_datapoint(episode_data, step_index))
                datapoints.extend(ep_datapoints)
            except Exception as e:
                print(f"Error loading episode {ep_idx} in shard {idx}: {e}")
                continue
        return datapoints
