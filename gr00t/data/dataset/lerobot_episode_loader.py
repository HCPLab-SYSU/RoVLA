...
# LEROBOT_TASKS_FILENAME = "tasks_raw.jsonl"
...
class LeRobotEpisodeLoader:
    ...
    def __init__(
        ...
        # After self._load_metadata() and self._parse_and_validate_modality_configs, filter metadata.
    ...
    def _filter_metadata(self):
        valid_episodes_metadata = []
        invalid_episodes_metadata = []
        for episode_metadata in self.episodes_metadata:
            data_valid = True
            ...
            validate_file_list = [parquet_path]
            for image_key in image_keys:
                ...
                validate_file_list.append(video_path)
            for p in validate_file_list:
                if not p.exists():
                    data_valid = False
                    break
            if data_valid:
                valid_episodes_metadata.append(episode_metadata)
            else:
                invalid_episodes_metadata.append(episode_metadata)
        self.episodes_metadata = valid_episodes_metadata
        print(f"{len(invalid_episodes_metadata)} episodes from {self.dataset_path} are invalid, and will be ignored")
    def _load_metadata(self) -> None:
        ...
        with open(tasks_path, "r") as f:
            ...
            if USE_TASK_LIST:
                self.tasks_map = {task["task_index"]: task.get("task_list", [task["task"]]) for task in tasks_data}
            else:
    ...
    def __getitem_once__(self, idx: int) -> pd.DataFrame:
        """
