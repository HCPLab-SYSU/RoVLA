...
class Gr00tTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.key_module_lr_ratios = kwargs.pop("key_module_lr_ratios", {}) #mapping from module key name to lr ratio
        self.key_module_lr_ratios = self.key_module_lr_ratios or {}
        super().__init__(
            *args,
            **kwargs,
    ...
    def create_optimizer(self):
        """
        扩展默认 optimizer 创建逻辑，支持对特定模块设置学习率倍数。
        """
        opt_model = self.model_wrapped if self.is_model_parallel else self.model

        if self.optimizer is None:
            # 获取需要 weight decay 的参数名（官方函数）
            ...
            # 收集所有参数，并按模块分组
            named_params = [(n, p) for n, p in opt_model.named_parameters() if p.requires_grad]
            ...
            # 1. 为特殊模块创建参数组（带 decay / no_decay 分离）
            for module_name, lr_ratio in self.key_module_lr_ratios.items():
                module_params = [(n, p) for n, p in named_params if module_name in n]
                module_key_name = set(['.'.join(n.split('.')[:3]) for n, _ in module_params])
                print(f"Setting learning rate {base_lr * lr_ratio} for module {module_key_name}")
                if not module_params:
                    continue

                # 分成 decay 和 no_decay 两组
                decay_p = [p for n, p in module_params if n in decay_parameters]
                no_decay_p = [p for n, p in module_params if n not in decay_parameters]
                ...
                assigned_params.update(n for n, _ in module_params)

            # 2. 剩余参数使用 base_lr
            remaining_params = [(n, p) for n, p in named_params if n not in assigned_params]
            decay_p = [p for n, p in remaining_params if n in decay_parameters]
            no_decay_p = [p for n, p in remaining_params if n not in decay_parameters]
            ...
            # 使用默认优化器类和 kwargs（兼容 AdamW, GaLore, LOMO 等）
            if self.optimizer_cls_and_kwargs is not None:
                optimizer_cls, optimizer_kwargs = self.optimizer_cls_and_kwargs
            else:
                optimizer_cls, optimizer_kwargs = self.get_optimizer_cls_and_kwargs(self.args, opt_model)

            # 处理特殊优化器（如 GaLore, LOMO）
