...
import torch
from transformers import AutoConfig, AutoModel, Qwen3VLForConditionalGeneration
from transformers.feature_extraction_utils import BatchFeature


class QwenVLBackbone(torch.nn.Module):
    def __init__(
        self,
        ...
        """
        QwenVLBackbone is to generate n_queries to represent the future action hidden states.
        Args:
            model_name: Qwen/Qwen3-VL-2B-Instruct or other QwenVL models
        ...
        if model_name == "Qwen/Qwen3-VL-2B-Instruct":
            assert use_flash_attention, (
                f"{model_name} requires flash attention by default"
            )
            assert load_bf16, f"{model_name} requires bfloat16 by default"
            module_path = os.path.join(os.path.dirname(__file__), "Qwen/Qwen3-VL-2B-Instruct")
    ...
    def post_init_from_pretrained(self, model_name):
        ...
        print(f"Loaded QwenVL backbone weights from {model_name}")
