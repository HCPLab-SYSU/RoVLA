...
def build_processor(model_name: str, transformers_loading_kwargs: dict) -> ProcessorMixin:
    if model_name == "nvidia/Eagle-Block2A-2B-v2":
        vlm_model_path = os.path.join(
            os.path.dirname(__file__), "..", "modules", "nvidia", "Eagle-Block2A-2B-v2"
        )
    elif model_name == "OpenGVLab/InternVL3_5-2B-HF":
        vlm_model_path = os.path.join(
            os.path.dirname(__file__), "..", "modules", "OpenGVLab/InternVL3_5-2B-HF"
        )
    elif model_name == "Qwen/Qwen3-VL-2B-Instruct":
        vlm_model_path = os.path.join(
            os.path.dirname(__file__), "..", "modules", "Qwen/Qwen3-VL-2B-Instruct"
        )
    else:
        raise Exception(f"Processor for {model_name} not supported")
    return AutoProcessor.from_pretrained(vlm_model_path, **transformers_loading_kwargs)
...
class Gr00tN1d6DataCollator:
    def __init__(
        self,
        model_name: str,
        model_type: Literal["eagle", "internvl", "qwen_vl"] = "eagle",
...
class Gr00tN1d6Processor(BaseProcessor):
    ...
    def _apply_vlm_processing(self, images: np.ndarray, language: str) -> BatchFeature:
        ...
        add_generation_prompt = self.model_type not in ["eagle"]
