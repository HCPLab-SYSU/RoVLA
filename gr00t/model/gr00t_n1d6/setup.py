...
class Gr00tN1d6Pipeline(ModelPipeline):
    ...
    def _create_model(self):
        ...
        if self.config.training.start_from_checkpoint is not None:
            ...
            # 检查是否有任意匹配的文件存在
            if any([len(glob.glob(os.path.join(self.config.training.start_from_checkpoint, pattern)))>0 for pattern in patterns]):
                model, loading_info = AutoModel.from_pretrained(
                    ...
                    use_pgd=self.config.model.use_pgd,
                    use_consistency_learning=self.config.model.use_consistency_learning,
                    add_condition_noise_sampler=self.config.model.add_condition_noise_sampler,
                ...
                model = self.model_class(model_config, transformers_loading_kwargs=self.transformers_loading_kwargs)
                if hasattr(model_config, "backbone_model_pretrained_path") and model_config.backbone_model_pretrained_path is not None:
                    model.backbone.post_init_from_pretrained(model_config.backbone_model_pretrained_path)
                logging.info(f"Loaded backbone from {model_config.backbone_model_pretrained_path} successfully.")
        else:
