...
- QUATERNION FORMAT: Unified to xyzw [w, x, y, z] throughout the entire script.
...
def main():
    parser = argparse.ArgumentParser(
        description="PANDA Real-Robot GR00T Policy Evaluation (Franky + RealSense)"
    )
    ...
    try:
        ...
        print(f"Starting evaluation loop with max_step={cfg.max_step}. Press Ctrl+C to stop.")
        print(f"Robot dynamics (velocity_rel): {cfg.control.velocity_rel}")
        ...
        while not done:
            ...
            for cmd in cmd_list:  # Limit horizon to 10 steps
                tic = time.time()
