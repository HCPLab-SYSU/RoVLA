...
def is_libero_env(env_name: str) -> bool:
    """Check if environment is a LIBERO environment"""
    return env_name.startswith("libero_")

def is_liberoplus_env(env_name: str) -> bool:
    """Check if environment is a LIBEROPLUS environment"""
    return env_name.startswith("liberoplus_")
def is_robotwin_env(env_name: str) -> bool:
    """Check if environment is a RobotWin environment"""
    return env_name.startswith("robotwin_")
def get_embodiment_tag_from_env_name(env_name: str) -> EmbodimentTag:
    ...
    if is_liberoplus_env(env_name):
        return EmbodimentTag.LIBEROPLUS_PANDA
    if is_robotwin_env(env_name):
        return EmbodimentTag.ROBOTWIN_ALOHA_AGILEX
