    ...
    **Important**: If you are using other environments, custom robots, or building new environments,
    ...
    def check_observation(self, observation: dict[str, Any]) -> None:
        ...
        for video_key in modality_configs["video"].modality_keys:
            ...
            if  video_key=="ego_view" and "video.ego_view_bg_crop_pad_res256_freq20" in observation:
                parsed_key = f"video.ego_view_bg_crop_pad_res256_freq20"
            else:
                parsed_key = f"video.{video_key}"
            assert parsed_key in observation, f"Video key '{parsed_key}' must be in observation, but observed: {observation.keys()}"
            ...
            for key in self.policy.modality_configs[modality].modality_keys:
                if modality == "language":
                    ...
                    # /PATCH
                        ...
                        parsed_key = key
                elif modality == "video" and key=="ego_view" and "video.ego_view_bg_crop_pad_res256_freq20" in observation:
                    parsed_key = f"{modality}.ego_view_bg_crop_pad_res256_freq20"
                else:
