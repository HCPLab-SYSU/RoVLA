...
# ================== 配置区（请按需修改） ==================
data_folder = "/vla/users/luojingzhou/data/checkpoints/GR00TN-1.6-InterVL3.5-3B/liberoplus/liberoplus_all_6w_pgd_step1_wonoiseadv_epsilon0d03_cslearning_tasklist/checkpoint-60000/eval_liberoplus/all/rollout_video"
...
def scan_tasks():
    ...
    for item in BASE_DIR.iterdir():
        ...
        for vid in mp4s:
            name = vid.name
            if name.endswith('_1.mp4') or name.endswith('1.mp4'):
                success_video = vid
            elif name.endswith('_0.mp4') or name.endswith('0.mp4'):
                fail_video = vid
            else:
...
def index():
    ...
    status_filter = request.args.get('status', 'all')  # all, success, fail, unknown
...
@app.route('/download/video_raw/<task_name>')
def download_video_raw(task_name):
    """下载原始视频文件（保留原始文件名）"""
    if task_name not in task_registry:
        abort(404)
    video_path = task_registry[task_name]["video_path"]
...
if __name__ == '__main__':
    ...
    print("="*70)
    app.run(host='127.0.0.1', port=45668, debug=False, threaded=True)